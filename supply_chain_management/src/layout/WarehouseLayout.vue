<template>
    <div class="warehouse-wrapper flex min-h-screen bg-[#F8FAFC] font-sans w-full overflow-x-hidden relative">
        ...
        <aside :class="[
            ...
            <div class="flex-1 py-8 w-full overflow-y-auto custom-scrollbar md:overflow-visible">
                <nav class="flex flex-col items-center gap-4 w-full">
                    ...
                    <button v-for="menu in warehouseMenus" :key="menu.id" @click="switchTab(menu.id)"
                        v-tooltip.right="menu.name"
                        class="w-14 h-14 rounded-2xl flex items-center justify-center transition-all duration-300 mx-auto group shrink-0"
                        ...
                        <i :class="['pi', menu.icon, 'text-xl', 'transition-transform', 'group-hover:scale-110']"></i>
                    </button>
            ...
            <div
                ...
                <button
                    class="w-10 h-10 flex items-center justify-center text-slate-400 hover:text-slate-700 transition-colors">
        ...
        <main class="flex-1 ml-0 md:ml-[136px] p-4 md:p-8 lg:p-10 max-w-[1400px] w-full min-h-screen pt-20 md:pt-8">
            ...
            <header
                ...
                <div>
                    <h2 class="text-xs font-bold uppercase tracking-widest text-slate-400 mb-1">Warehouse Management
                    </h2>
            ...
            <div class="relative w-full">
                <Received v-if="activeTab === 'received'" />
                <LogLogistick v-if="activeTab === 'logistick'" />
                <Packaging v-if="activeTab === 'packaging'" />
                <StockPt v-if="activeTab === 'stock_pt'" />
                <StockCv v-if="activeTab === 'stock_cv'" />
                <RawStock v-if="activeTab === 'raw_stock'" />
            </div>
...
import Received from '../components/warehouse/Received.vue'
import LogLogistick from '../components/warehouse/LogLogistick.vue'
import Packaging from '../components/warehouse/Packaging.vue'
import StockPt from '../components/warehouse/StockPt.vue'
import StockCv from '../components/warehouse/StockCv.vue'
import RawStock from '../components/warehouse/RawStock.vue'

const router = useRouter()
const activeTab = ref('received')
const isMobileMenuOpen = ref(false)

// Array Data-Driven untuk Menu
const warehouseMenus = [
    { id: 'received', name: 'Penerimaan Barang (Received)', icon: 'pi-inbox' },
    { id: 'logistick', name: 'Log Logistik & Pengemasan', icon: 'pi-box' },
    { id: 'packaging', name: 'Transfer Hasil Produksi (R&D)', icon: 'pi-sync' },
    { id: 'stock_pt', name: 'Data Stok - PT', icon: 'pi-building' },
    { id: 'stock_cv', name: 'Data Stok - CV', icon: 'pi-briefcase' },
    { id: 'raw_stock', name: 'Daftar Stok Bahan Baku', icon: 'pi-list' }
]

// Judul Dinamis berdasarkan Tab aktif
const currentMenuName = computed(() => {
    const menu = warehouseMenus.find(m => m.id === activeTab.value)
    return menu ? menu.name : 'Warehouse Management'
})

const switchTab = (tab) => {
    activeTab.value = tab
    isMobileMenuOpen.value = false // Auto tutup saat dibuka di HP
}
